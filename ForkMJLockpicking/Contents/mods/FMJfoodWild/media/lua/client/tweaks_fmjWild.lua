require("ItemTweaker_Copy_fmj");

TweakItem("Base.WildGarlic","UnhappyChange","10");
TweakItem("Base.WildGarlic","HungerChange","-8");
TweakItem("Base.WildGarlic","Spice","true");
TweakItem("Base.WildGarlic","EvolvedRecipe","Soup:3;Pie:1;Stir fry:3;Stir fry Griddle Pan:3;Burger:1;Salad:2;Roasted Vegetables:3;RicePot:3;RicePan:3;PastaPot:3;PastaPan:3");
TweakItem("Base.WildGarlic","Carbohydrates","9");
TweakItem("Base.WildGarlic","Proteins","4.2");
TweakItem("Base.WildGarlic","Lipids","0.3");
TweakItem("Base.WildGarlic","Calories","88");
TweakItem("Base.WildGarlic","CantBeFrozen","true");

TweakItem("Base.HuntingMag1","TeachedRecipes","Make Snare Trap;Make Mouse Trap");
