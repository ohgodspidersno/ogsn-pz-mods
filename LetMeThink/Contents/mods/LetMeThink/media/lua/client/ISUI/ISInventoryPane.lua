...
function ISInventoryPane:doGrabOnJoypadSelected()
    ...
    if self.parent.onCharacter then
        ISInventoryPaneContextMenu.onPutItems(contextMenuItems, self.player);
    else
...
function ISInventoryPane:refreshContainer()
    ...
    for i = 0, it:size()-1 do
		...
		if instanceof(item, "Clothing") and (item:getBodyLocation() == "ZedDmg" or item:getBodyLocation() == "Bandage") then
			add = false;
		end
		if add then
