...
ISInventoryPaneContextMenu.doClothingRecipeMenu = function(playerObj, clothing, items, context)
  ...
  if clothing:getFabricType() and instanceof(clothing, "Clothing") then
    local option = context:addOption(getRecipeDisplayName("Rip sheets"), playerObj, ISInventoryPaneContextMenu.onRipClothing, items)
...
ISInventoryPaneContextMenu.doWearClothingMenu = function(player, clothing, items, context)
  ...
  for i = 1, wornItems:size() do
    ...
    if (clothing:getBodyLocation() == wornItem:getLocation()) or bodyLocationGroup:isExclusive(clothing:getBodyLocation(), wornItem:getLocation()) then
  ...
  for i = 0, recipe:size() - 1 do
    ...
    if numberOfTimes ~= 1 then
      ...
      if playerObj:isDriving() then
        ...
        local subOption = subMenuCraft:addOption(getText("ContextMenu_One"), selectedItem, ISInventoryPaneContextMenu.OnCraft, recipe:get(i), player, false);
        local tooltip = ISInventoryPaneContextMenu.addToolTip();
        ...
        if numberOfTimes > 1 then
          local subOption = subMenuCraft:addOption(getText("ContextMenu_AllWithCount", numberOfTimes), selectedItem, ISInventoryPaneContextMenu.OnCraft, recipe:get(i), player, true);
          setmetatable(tooltip, CraftTooltip)
          tooltip.recipe = recipe:get(i)
          -- add it to our current option
          tooltip:setName(recipe:get(i):getName());
          ...
          local subOption = subMenuCraft:addOption(getText("ContextMenu_All"), selectedItem, ISInventoryPaneContextMenu.OnCraft, recipe:get(i), player, true);
          setmetatable(tooltip, CraftTooltip)
          tooltip.recipe = recipe:get(i)
          -- add it to our current option
          tooltip:setName(recipe:get(i):getName());
