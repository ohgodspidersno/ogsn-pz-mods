...
ISInventoryPaneContextMenu.createMenu = function(player, isInPlayerInventory, items, x, y, origin)
  ...
  if isInPlayerInventory and not unequip and not ISInventoryPaneContextMenu.isAllFav(items) then
...
ISInventoryPaneContextMenu.addItemInEvoRecipe = function(subMenuRecipe, baseItem, evoItem, extraInfo, evorecipe2, player)
  ...
  if instanceof(evoItem, "DrainableComboItem") then
    if (baseItem:getFreezingTime() > 0) then
      option.notAvailable = true;
      local tooltip = ISInventoryPaneContextMenu.addToolTip();
      tooltip.description = getText("ContextMenu_CantAddFrozenFood");
      option.toolTip = tooltip;
    end
  elseif evoItem:getFreezingTime() > 0 then
    option.notAvailable = true;
    local tooltip = ISInventoryPaneContextMenu.addToolTip();
    tooltip.description = getText("ContextMenu_CantAddFrozenFood");
    option.toolTip = tooltip;
...
ISInventoryPaneContextMenu.isAllFav = function (items)
  ...
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        if i2 ~= 1 and not k2:isFavorite() then
          fav = false;
        end
...
ISInventoryPaneContextMenu.buildFixingMenu = function(brokenObject, player, fixing, fixOption, subMenuFix)
  local tooltip = ISInventoryPaneContextMenu.addToolTip();
  tooltip.description = "";
  ...
  for i = 0, fixing:getFixers():size() - 1 do
    ...
    local subOption = ISInventoryPaneContextMenu.addFixerSubOption(brokenObject, player, fixing, fixer, subMenuFix);
    local add = "";
...
ISInventoryPaneContextMenu.getContainers = function(character)
  ...
  for i, v in pairs(getPlayerInventory(playerNum).inventoryPane.inventoryPage.backpacks) do
    containerList:add(v.inventory);
  end
  for i, v in pairs(getPlayerLoot(playerNum).inventoryPane.inventoryPage.backpacks) do
    containerList:add(v.inventory);
  end
  return containerList;
end

ISInventoryPaneContextMenu.addFixerSubOption = function(brokenObject, player, fixing, fixer, subMenuFix)
  ...
  if usedItem then
    ...
    fixOption = subMenuFix:addOption(fixer:getNumberOfUse() .. " " .. getItemText(usedItem:getName()), brokenObject, ISInventoryPaneContextMenu.onFix, player, fixing, fixer);
  else
    fixOption = subMenuFix:addOption(fixer:getNumberOfUse() .. " " .. fixer:getFixerName(), brokenObject, ISInventoryPaneContextMenu.onFix, player, fixing, fixer);
...
ISInventoryPaneContextMenu.onFix = function(brokenObject, player, fixing, fixer)
  ISTimedActionQueue.add(ISFixAction:new(getSpecificPlayer(player), brokenObject, 60, fixing, fixer));
end

ISInventoryPaneContextMenu.onDryMyself = function(towels, player)
  for i, k in pairs(towels) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISInventoryPaneContextMenu.dryMyself(k2, player);
          break;
        end
      end
    else
...
ISInventoryPaneContextMenu.onApplyBandage = function(bandages, bodyPart, player)
  for i, k in pairs(bandages) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISInventoryPaneContextMenu.applyBandage(k2, bodyPart, player);
          break;
        end
      end
    else
  ...
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISInventoryPaneContextMenu.readItem(k2, player);
          break;
        end
      end
    else
...
ISInventoryPaneContextMenu.onUnEquip = function(items, player)
  --~ 	getSpecificPlayer(ISContextMenu.globalPlayerContext):setClothingItem_Torso(nil);
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISInventoryPaneContextMenu.unequipItem(k2, player);
        end
      end
    else
  ...
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISInventoryPaneContextMenu.wearItem(k2, player);
          break;
        end
      end
    else
  ...
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISTimedActionQueue.add(ISInventoryTransferAction:new(getSpecificPlayer(player), k2, getPlayerInventory(player).inventory, getPlayerLoot(player).inventory));
        end

      end
    else
      ISTimedActionQueue.add(ISInventoryTransferAction:new(getSpecificPlayer(player), k, getPlayerInventory(player).inventory, getPlayerLoot(player).inventory));
    end
  end
end

ISInventoryPaneContextMenu.canMoveTo = function(items, dest)
  if instanceof(dest, "InventoryContainer") then
    for i, item in ipairs(items) do
      if item == dest then return nil end
      if dest:getInventory():contains(item) then return nil end
      if dest:getType() == "KeyRing" and not instanceof(item, "Key") then return nil end
    end
    return dest
  end
...
ISInventoryPaneContextMenu.addDynamicalContextMenu = function(selectedItem, context, recipe, player, containerList)
  ...
  for i = 0, recipe:size() - 1 do
    ...
    if numberOfTimes ~= 1 then
      local subMenuCraft = context:getNew(context);
...
ISInventoryPaneContextMenu.OnPrimaryWeapon = function(items, player)
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 and (instanceof(k2, "HandWeapon") and k2:getCondition() > 0) or (instanceof(k2, "InventoryItem") and not instanceof(k2, "HandWeapon")) then
          ISInventoryPaneContextMenu.equipWeapon(k2, true, false, player);
          break;
        end
      end
    elseif (instanceof(k, "HandWeapon") and k:getCondition() > 0) or (instanceof(k, "InventoryItem") and not instanceof(k, "HandWeapon")) then
  ...
  --~ 	end
  --~ 	ISInventoryPaneContextMenu.equipWeapon(weapon, true);
end
  ...
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISInventoryPaneContextMenu.takePill(k2, player);
          break;
        end
...
ISInventoryPaneContextMenu.OnTwoHandsEquip = function(items, player)
  ...
  if not instanceof(items[1], "InventoryItem") then
    -- first in a list is a dummy duplicate, so ignore it.
    weapon = items[1].items[2];
  end
  ISInventoryPaneContextMenu.equipWeapon(weapon, false, true, player);
end
...
ISInventoryPaneContextMenu.OnSecondWeapon = function(items, player)
  ...
  ISInventoryPaneContextMenu.equipWeapon(weapon, false, false, player);
end
  ...
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISInventoryPaneContextMenu.information(k2);
          break;
        end
  ...
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 then
          ISInventoryPaneContextMenu.eatItem(k2, percentage, player);
          break;
        end
      end
    else
...
ISInventoryPaneContextMenu.dropItem = function(item, player)
  ...
  local lootInv = getPlayerLoot(player).inventory;
...
ISInventoryPaneContextMenu.transferItems = function(items, playerInv, player)
  for i, k in pairs(items) do
    if not instanceof(k, "InventoryItem") then
      for i2, k2 in ipairs(k.items) do
        -- first in a list is a dummy duplicate, so ignore it.
        if i2 ~= 1 and k2:getContainer() ~= playerInv and k2:getContainer() ~= null then
          ISTimedActionQueue.add(ISInventoryTransferAction:new(getSpecificPlayer(player), k2, k2:getContainer(), playerInv));
        end
      end
    elseif k:getContainer() ~= playerInv and k:getContainer() ~= null then
...
ISInventoryPaneContextMenu.onGrabHalfItems = function(items, player)
  --~     local playerInv = getPlayerInventory(ISContextMenu.globalPlayerContext);
  local playerInv = getPlayerInventory(player).inventory;
...
ISInventoryPaneContextMenu.onTransferWater = function(items, itemFrom, itemTo, player)
  ...
  if waterStorageAvailable < waterStorageNeeded then
    ...
    local waterInB = itemFrom:getUsedDelta() / itemFrom:getUseDelta();
    local waterToMove = waterInB - waterStorageAvailable;

    itemFromEndingDelta = waterToMove * itemFrom:getUseDelta();
    itemToEndingDelta = 1;
  end
