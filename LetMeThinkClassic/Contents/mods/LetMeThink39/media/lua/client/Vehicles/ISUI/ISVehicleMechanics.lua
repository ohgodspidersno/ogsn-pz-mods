...
function ISVehicleMechanics.onCheatGetKey(playerObj, vehicle)
  if isClient() then
    ...
    playerObj:getInventory():AddItem(vehicle:createVehicleKey())
  end
end
...
function ISVehicleMechanics.onCheatHotwire(playerObj, vehicle, hotwired, broken)
  if isClient() then
    ...
    vehicle:cheatHotwire(hotwired, broken)
  end
end
...
ISVehicleMechanics.OnMechanicActionDone = function(success, vehicleId, partId, itemId, installing)
  if success then
    local chr = getPlayer(0);
    ...
    if installing then
      ...
      chr:addMechanicsItem(part:getInventoryItem():getID() .. vehicle:getMechanicalID() .. "0", part, getGameTime():getCalender():getTimeInMillis());
    end
  end
