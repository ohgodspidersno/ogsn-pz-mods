...
function ISVehicleMenu.showRadialMenu(playerObj)
  ...
  if vehicle:isDriver(playerObj) and vehicle:hasHeadlights() then
    ...
    elseif vehicle:getBatteryCharge() > 0.0 then
  ...
  if (not isClient() or getServerOptions():getBoolean("SleepAllowed")) and not vehicle:isEngineRunning() then
    local doSleep = true;
  ...
  elseif vehicle:isEngineRunning() then
    menu:addSlice(getText("ContextMenu_NoSleepEngineRunning"), getTexture("media/ui/vehicles/vehicle_sleep.png"), nil, playerObj, vehicle);
  end
...
function ISVehicleMenu.onToggleHeadlights(playerObj)
  ...
  if isClient() then
    ...
    vehicle:setHeadlightsOn(not vehicle:getHeadlightsOn())
  end
end
...
function ISVehicleMenu.onToggleTrunkLocked(playerObj)
  ...
  if isClient() then
    ...
    vehicle:setTrunkLocked(not vehicle:isTrunkLocked());
  end
end
